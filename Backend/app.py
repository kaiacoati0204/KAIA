...
async def reward_objetivo(conn, session_id, desde, ate):
    ...
    CHUTE não é retomar: erro rápido demais para o ritmo do aluno (z < -3 na régua da regra);
    pagá-lo ensina o bandit que a intervenção fechada no clique funcionou (Baker 2007). Sem
    régua antes do disparo, não julga. Devolve None se não houve resposta e a sessão foi
    encerrada dentro da janela (parar pode ser o sucesso de alerta_fadiga/pausa_ativa)."""
    linhas = await conn.fetch(
        """
        select payload from session_events
